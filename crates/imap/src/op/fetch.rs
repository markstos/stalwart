...
impl AsImapDataItem for ArchivedMessageMetadata {
        ...
        if (part.flags & PART_ENCODING_PROBLEM) == 0 {
            ...
            Ok(match &part.body {
                ArchivedMetadataPartType::Text | ArchivedMetadataPartType::Html => {
                    // RFC 3516: BINARY fetch only strips content-transfer-encoding.
                    // Do NOT convert charset — the client handles that via BODYSTRUCTURE.
    ...
    fn binary_size(&self, decoded: &DecodedParts<'_>, sections: &[u32]) -> Option<usize> {
        ...
        match &part.body {
            ArchivedMetadataPartType::Text | ArchivedMetadataPartType::Html => {
                // RFC 3516: size must match what BINARY fetch returns (no charset conversion).
                decoded
                    .transfer_decoded_contents(message_id, part)
                    .map(|p| p.len())
                    .unwrap_or_default()
